...
static AVCodecContext *pVideoCodecCtx_vod = 0; // ffmpeg编码视频上下文pVideoCodecCtx_vod

void audio_callback_vod(void *userdata, Uint8 *stream, int len) // 音频回调函数，SDL_OpenAudio使用后会无限循环使用该函数，直到使用SDL_closeAudio，如果不close则SDL_Quit无限阻塞
{
	...
	//{if((audio_pace+2)>decode_audio_pace)Sleep(too_fast_wait);}//如果缓冲很不够了，强制停顿
	// else
	//{if((video_pace+2)>decode_video_pace)Sleep(too_fast_wait);}//如果缓冲很不够了，强制停顿//视频播放播完了，可能是mp3电台节目

	if ((audio_pace + 7) > decode_audio_pace)
		Sleep(2); // 如果缓冲不够了，稍微做出看不出的停顿
	...
	if ((read_count == bufnumber / 4 || read_count == (bufnumber * 3 / 4)) && read_this_buf->read_buf_index < len)
	{
		next_write = true;
	} // 可以让写服务继续写了
...
void process_video_function_vod()
{
	...
	// 步骤七：对拿到的原始数据格式进行缩放转换为指定的格式高宽大小  AV_PIX_FMT_YUV420P        pVideoCodecCtx_vod->pix_fmt AV_PIX_FMT_RGBA
	...
	av_image_fill_arrays(pAVFrameRGB32->data, pAVFrameRGB32->linesize, outBuffer, AV_PIX_FMT_YUV420P, pVideoCodecCtx_vod->width, pVideoCodecCtx_vod->height, 1); // pAVFrame32的data指针指向了outBuffer
	...
	if (pVideoCodecCtx_vod->width > my_width)
		set_width = my_width;
	else
		set_width = pVideoCodecCtx_vod->width; // 设置sdl屏幕长宽
	if (pVideoCodecCtx_vod->height > my_height)
		set_height = my_height;
	else
		set_height = pVideoCodecCtx_vod->height;
	...
	while (true)
	{
		...
		while (video_pace == decode_video_pace && !write_video_buff_over)
		{
			SDL_PollEvent(&event);
			Sleep(100);
		} // 如果还没接受到足够多的数据，进行等待
...
void show_moive_vod() // 点播
{
	...
	write_video_buff_over = false; // 初始化，都没播完
	...
	else
	{
		...
		if (avcodec_open2(pVideoCodecCtx_vod, vCodec, &pAVDictionary))
		{
		...
		// 显示视频相关的参数信息（编码上下文）
		cout << "比特率:" << pVideoCodecCtx_vod->bit_rate << endl;
		...
		cout << "格式:" << pVideoCodecCtx_vod->pix_fmt << endl; // 格式为0说明是AV_PIX_FMT_YUV420P
		cout << "宽:" << pVideoCodecCtx_vod->width << "\t高：" << pVideoCodecCtx_vod->height << endl;
		cout << "文件分母:" << pVideoCodecCtx_vod->time_base.den << "\t文件分子:" << pVideoCodecCtx_vod->time_base.num << endl;
	...
	else // 找到音频索引了，进行音频配置
	{
		...
		desired_spec.callback = audio_callback_vod; // 设置音频回调函数为audio_callback_vod()
	...
	if (videoIndex >= 0)
		CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)process_video_function_vod, NULL, NULL, NULL); // 如果有视频，开始视频线程部署并播放
	...
	while (true)
	{
		...
		if (pPacket->stream_index == videoIndex) // 如果当前数据包是视频包，将视频包存入视频包缓存，等待要播放该帧所在包时再解码
		{
			...
			while (avcodec_receive_frame(pVideoCodecCtx_vod, vframe[write_v_count]) >= 0) // 解码开始
			{
				write_v_count++;
	...
	write_video_buff_over = true; // 视频解码结束
	...
	if (pVideoCodecCtx_vod)
	{
		avcodec_close(pVideoCodecCtx_vod);
		pVideoCodecCtx_vod = 0;
	}
	if (pAVFormatContext)
	{
	...
	// SDL_DestroyRenderer(pSDLRenderer);//销毁sdl渲染器,此句在处理视频的线程中执行
	// SDL_DestroyWindow(pSDLWindow);cout<<"即将执行SDL_Quit()\n";// 销毁sdl窗口,此句在处理视频的线程中执行
	SDL_Quit(); // 退出SDL
}